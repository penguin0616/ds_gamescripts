...
AddGameDebugKey(KEY_X, function()
    if TheInput:IsKeyDown(KEY_SHIFT) then
		if SaveGameIndex:IsModeShipwrecked() then
			c_spawn("shipwrecked_exit")
		else
			c_spawn("shipwrecked_entrance")
		end
    end
end)
...
AddGameDebugKey(KEY_F12, function()
    --Spawns a lot of prefabs around you in rings.
    local items = {"coconut"} --Which items spawn. 
    local player = GetPlayer()
    local pt = Vector3(player.Transform:GetWorldPosition())
    local theta = math.random() * 2 * math.pi
    local numrings = 8 --How many rings of stuff you spawn
    local radius = 1 --Initial distance from player
    local radius_step_distance = 2 --How much the radius increases per ring.
    ...
    for i = 1, numrings do
        ...
        for i = 1, numitems do
            ...
            local spawn = SpawnPrefab(GetRandomItem(items))
            spawn.Transform:SetPosition( wander_point.x, wander_point.y, wander_point.z ) 
            spawn.components.inventoryitem:OnHitGround()   
            --end
            theta = theta - (2 * PI / numitems)
        end
        radius = radius + radius_step_distance
...
-- AddGameDebugKey(KEY_E, function()
--     if TheInput:IsKeyDown(KEY_CTRL) then
--         GetWorld().components.volcanomanager:StartEruption(60.0, 60.0, 60.0, 1/8)
--         return true         
--     end
-- end)
...
AddGlobalDebugKey(KEY_L, function()
	if not TheInput:IsKeyDown(KEY_CTRL) then -- ctrl L closes log!
		if TheInput:IsKeyDown(KEY_SHIFT) then
			GetSeasonManager():StartWet()
		end
		if GetWorld().components.seasonmanager:IsWetSeason() then
			GetWorld().components.seasonmanager:StartHurricaneStorm(100)
		else
			GetWorld().components.seasonmanager:StartHurricaneTease(10)
		end
		return true
	end
end)
