    ...
    elseif target_ent then
        ...
        if not actions then
            ...
            elseif (boatitem and boatitem:IsValid()) and not equipitem then
                actions = self:GetEquippedItemActions(target_ent, boatitem)
            elseif (equipitem and equipitem:IsValid()) and (boatitem and boatitem:IsValid()) then

                local equip_act = self:GetEquippedItemActions(target_ent, equipitem)
     ...
     if position then 
        ...
        elseif isCursorWet and not isBoating and not TheInput:ControllerAttached()  and not interactingWithBoat  and (target_ent == nil or target_ent:HasTag("aquatic")) then 
           ...
           --[[
            ...
            if stoppos then 
                --stoppos.x, stoppos.y, stoppos.z = GetWorld().Map:GetTileCenterPoint(stoppos.x,0, stoppos.z)
                actions = { BufferedAction(self.inst, nil, ACTIONS.WALKTO, nil, stoppos) }
            end 
            ]]
        end
    end 
	...
	if not target then        
		...
		if useitem then
			if (useitem.components.equippable and not useitem.components.equippable.isequipped )
               ...
               or useitem.components.poisonhealer then
				ignore_player = false
			end
		end
