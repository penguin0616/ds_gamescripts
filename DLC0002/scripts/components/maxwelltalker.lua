...
function MaxwellTalker:OnCancel()
	...
	if self.inst.speech.disableplayer and self.inst.wilson then
	    if self.inst.wilson.sg.currentstate.name == "sleep" then		
		    ...
		    if self.inst.speech.disappearanim and type(self.inst.speech.disappearanim) == "string" then
		    ...
		    elseif self.inst.speech.disappearanim and type(self.inst.speech.disappearanim) == "function" then
		    	self.inst.speech.disappearanim(self.inst)
...
function MaxwellTalker:Initialize(nohide)
	...
	if self.inst.speech and self.inst.speech.disableplayer then
        ...
        if self.cleartrees then 
	        local playerPos = Vector3(self.inst.wilson.Transform:GetWorldPosition())
	        local camPos = Vector3(TheSim:GetCameraPos())

	        local camToPlayer = playerPos - camPos 
	        local distance = camToPlayer:Length() + 4

	        local tags = {"tree"}
	        local ents  = TheSim:FindEntities(playerPos.x, playerPos.y, playerPos.z, distance, tags)

	        for i = 1, #ents do 
	        	local ent = ents[i]
	        	local pos = Vector3(ent.Transform:GetWorldPosition())

	        	local entToPlayer = playerPos - pos 
	        	local entToCam = camPos - pos 

	        	if entToCam:Length() < distance and entToPlayer:Length() < distance then 
	        		ent:Remove() 
	        	end 
	        end 
	    end 
...
function MaxwellTalker:DoTalk()	
	...
	if self.inst.speech then
		...
		if self.inst.speech and self.inst.speech.idleanim then 
			if self.inst.speech.appearanim then
				self.inst.AnimState:PushAnimation(self.inst.speech.idleanim, true)
			else
				self.inst.AnimState:PlayAnimation(self.inst.speech.idleanim, true)
			end 
		end
		...
		for k, section in ipairs(self.inst.speech) do --the loop that goes on while the speech is happening
			...
			if section.sectionfn then
				section.sectionfn(self.inst)
			end

			if section.anim then --If there's a custom animation it plays it here.
				if section.pushanim then
					self.inst.AnimState:PushAnimation(section.anim)
				else
	        ...
	        if section.string then --If there is speech to be said, it displays the text and overwrites any custom anims with the talking anims
	        	...
	        	if self.inst.speech and self.inst.speech.dialoganim then 
	        		if self.inst.speech.dialogpreanim then
	        			self.inst.AnimState:PushAnimation(self.inst.speech.dialoganim, true)
	        		else
	        			self.inst.AnimState:PlayAnimation(self.inst.speech.dialoganim, true)
	        		end
        		end
			...
			if section.string then	--If maxwell was talking it winds down here and stops the anim.
	       	...
	       	if self.inst.speech and self.inst.speech.idleanim then --goes to an idle animation
	       		if self.inst.speech.dialogpostanim then
	       			self.inst.AnimState:PushAnimation(self.inst.speech.idleanim, true)
	       		else
	       			self.inst.AnimState:PlayAnimation(self.inst.speech.idleanim, true)
	       		end
		...
		if self.inst.speech and self.inst.speech.disappearanim and type(self.inst.speech.disappearanim) == "string" then
            if SaveGameIndex:GetCurrentMode(Settings.save_slot) == "adventure" then
			    self.inst.SoundEmitter:PlaySound("dontstarve/maxwell/disappear_adventure")
			    local fx = SpawnPrefab("maxwell_smoke")
    			fx.Transform:SetPosition(self.inst.Transform:GetWorldPosition())
			...
			self.inst:ListenForEvent("animqueueover", function()  self.inst:Remove() end)
		elseif self.inst.speech and self.inst.speech.disappearanim and type(self.inst.speech.disappearanim) == "function" then
			self.inst.speech.disappearanim(self.inst)
		end
