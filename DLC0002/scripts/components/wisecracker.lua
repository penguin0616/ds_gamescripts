local Wisecracker = Class(function(self, inst)
    ...
    inst:ListenForEvent("bermudatriangleexit", function(inst, data)
        inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_WORMHOLE"))
    end)
    ...
    inst:ListenForEvent("crab_fail", function(inst, data)
        inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_CRAB_ESCAPE"))
    end)

    inst:ListenForEvent("trawl_full", function(inst, data)
        inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_TRAWL_FULL"))
    end)

    inst:ListenForEvent("boat_damaged", function(inst, data)
        inst.components.talker:Say(GetString(inst.prefab, data.message))
    end)

    inst:ListenForEvent("boostbywave", function(inst, data)
        if not inst.last_wave_boost_talk or GetTime() - inst.last_wave_boost_talk > TUNING.SEG_TIME * 3 then
            ...
            inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_WAVE_BOOST"))
        end
    end)

    inst:ListenForEvent("whalehuntlosttrail", function(inst, data)
        if data.washedaway then
            inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_WHALE_HUNT_LOST_TRAIL_SPRING"))
        else
            inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_WHALE_HUNT_LOST_TRAIL"))
        end
    end)

    inst:ListenForEvent("whalehuntbeastnearby", function(inst, data)
        inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_WHALE_HUNT_BEAST_NEARBY"))
    end)

    inst:ListenForEvent("treasureuncover", function(inst, data)
        inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_TREASURE_DISCOVER"))
    end)

    inst:ListenForEvent("magic_fail", function(inst, data)
        inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_MAGIC_FAIL"))
    end)
end)
