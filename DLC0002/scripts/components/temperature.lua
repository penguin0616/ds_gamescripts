...
function Temperature:OnUpdate(dt, applyhealthdelta)
	...
	if seasonmgr and self.inst.components.inventory then
		for k,v in pairs (self.inst.components.inventory.equipslots) do
			if v.components.heater then
				local heat = v.components.heater:GetEquippedHeat(self.inst)
				if v.components.heater.show_heat then
					v:PushEvent("get_heat")
				end
	...
	-- Volcano adds heat
	local lava_dist = math.huge
    if GetWorld().IsVolcano() then
		if GetWorld().components.volcanomanager:IsErupting() then
			ambient_delta = ambient_delta + TUNING.VOLCANORIM_ACTIVE_HEAT
		else
	    	local map = GetWorld().Map
	    	if map and map:IsTileGridValid() then
				local tx, ty = map:GetTileXYAtPoint(x, y, z)
				lava_dist = map:GetClosestTileDist(tx, ty, GROUND.VOLCANO_LAVA, TUNING.VOLCANORIM_LAVA_DIST)
				if lava_dist <= TUNING.VOLCANORIM_LAVA_DIST then
					ambient_delta = ambient_delta + TUNING.VOLCANORIM_LAVA_HEAT
				end
			end
		end
    end
    ...
    if GetWorld().IsVolcano() then
		if GetWorld().components.volcanomanager:IsErupting() then
			self.rate = TUNING.VOLCANORIM_ACTIVE_MULT * self.rate
		elseif lava_dist <= TUNING.VOLCANORIM_LAVA_DIST then
			self.rate = TUNING.VOLCANORIM_LAVA_MULT * (1 - (lava_dist / TUNING.VOLCANORIM_LAVA_DIST)) * self.rate
		end
    end
