...
local Hunter = Class(function(self, inst)
    ...
    self.alternate_beast_prefab =  { "warg", "spat" }
...
function Hunter:OnUpdate()
    ...
    if self.lastdirttime and GetSeasonManager() and (GetSeasonManager():IsSpring() or GetSeasonManager():IsGreenSeason()) then
...
function Hunter:ResetHunt(springwash)
    ...
    if GetSeasonManager() and (GetSeasonManager():IsSpring() or GetSeasonManager():IsGreenSeason()) then
        self:StartCooldown(TUNING.HUNT_SPRING_RESET_TIME)
    else
        self:StartCooldown(TUNING.HUNT_RESET_TIME)
    end
    if springwash then
        GetPlayer():PushEvent("huntlosttrail", {washedaway=true})
    else
        GetPlayer():PushEvent("huntlosttrail", {washedaway=false})
    end
...
function Hunter:SpawnHuntedBeast()
    ...
    if spawn_pt then
        if math.random() > self:GetAlternateBeastChance() then 
            ...
            self.huntedbeast = SpawnPrefab(GetRandomItem(self.alternate_beast_prefab))
        end
        if self.huntedbeast then
