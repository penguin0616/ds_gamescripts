    ...
    if item.prefab == self.inst.prefab then
        ...
        if self.maxsize >= newtotal then
            ...
            if self.inst.components.moisturelistener and item.components.moisturelistener then
                ...
                local itemmoisture = item.components.moisturelistener.moisture
                --DH: This is a not great fix for the case when you pick/harvest something off of something else that's wet 
                --In that case the picked items moisture inheriting is defferred by a couple frames and will never happen if the item is added to a stack in that time 
                --because the instance is removed here before the task fires  
                if item.targetMoisture then 
                    itemmoisture = item.targetMoisture
                end 
                self.inst.components.moisturelistener:Dilute(numberadded, itemmoisture)
            end
