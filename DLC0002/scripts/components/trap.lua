...
function Trap:CanTrap()
    local onwater = self.inst:GetIsOnWater()

    return not ((self.water and not onwater) or (not self.water and onwater))
end
...
function Trap:DoSpring()
    ...
    elseif self.target ~= nil then
        ...
        local x, y, z = self.inst.Transform:GetWorldPosition()
        local ents = TheSim:FindEntities(x, y, z, 2, nil, INLIMBO_TAGS)
            ...
            for i, v in ipairs(self.lootprefabs) do
                ...
                if loot ~= nil then
                    if inventory ~= nil then
                        inventory:GiveItem(loot, nil, Vector3(TheSim:GetScreenPos(pos:Get())))
                    else
