...
function Node:AddEntity(prefab, points_x, points_y, current_pos_idx, entitiesOut, width, height, prefab_list, prefab_data, rand_offset)
	AddEntity(prefab, points_x[current_pos_idx], points_y[current_pos_idx], entitiesOut, width, height, prefab_list, prefab_data, rand_offset)
end
...
function Node:SetTilesViaFunction(entities, width, height, spawnFn, world_gen_choices)
	...
	self.custom_tiles_data.data.spawnFn = spawnFn
	self.custom_tiles_data.data.world_gen_choices = world_gen_choices
...
function Node:PopulateVoronoi(spawnFn, entitiesOut, width, height, world_gen_choices, is_adventuremode)
	...
	if self.data.terrain_contents.countprefabs ~= nil then
		...
		for prefab, count in pairs(generate_these) do 	
			for id = 1, count do
				...
				if AddEntityCheck(prefab, points_x[current_pos_idx], points_y[current_pos_idx], entitiesOut, self.data.terrain_contents.prefabspawnfn) then
					local prefab_data = {}
	...
	--add a debugsign
	if DEBUGSIGNS_ENABLED and DEBUGSIGNS_ENABLED == true then
		--if current_pos_idx < #points_x and (not self.graph.debugsign or self.graph.debugsign == nil or self.graph.debugsign == false) then
		if not self.debugsign or self.debugsign == nil or self.debugsign == false then
			local prefab_data = { data = { debugtext = string.format("It says 'Welcome to %s - %s'", self.graph.id, self.id) }}
			local signx, signy = WorldSim:GetSiteCentroid(self.id)
			if current_pos_idx < #points_x and WorldSim:IsWater(WorldSim:GetTile(signx, signy)) then
				signx = points_x[current_pos_idx]
				signy = points_y[current_pos_idx]
				current_pos_idx = current_pos_idx + 1
			end
			self:AddEntity("homesign", {signx}, {signy}, 1, entitiesOut, width, height, prefab_list, prefab_data)
			--self.graph.debugsign = true
			self.debugsign = true
		end
	end
	...
	if self.data.terrain_contents.distributepercent and self.data.terrain_contents.distributeprefabs then
		...
		for current_pos_idx = current_pos_idx, #points_x  do
			...
			if math.random() < self.data.terrain_contents.distributepercent then
				...
				if prefab ~= nil and AddEntityCheck(prefab, points_x[current_pos_idx], points_y[current_pos_idx], entitiesOut, self.data.terrain_contents.prefabspawnfn) then
					local prefab_data = {}
					prefab_data.data = self.data.terrain_contents.prefabdata and self.data.terrain_contents.prefabdata[prefab] or nil			
					self:AddEntity(prefab, points_x, points_y, current_pos_idx, entitiesOut, width, height, prefab_list, prefab_data)				
				else
		...
		self:PopulateExtra(world_gen_choices, spawnFn, {points_type=points_type, points_x=points_x, points_y=points_y, idx_left=idx_left, entitiesOut=entitiesOut, width=width, height=height, prefab_list=prefab_list}, is_adventuremode)
	end

end


function Node:PopulateChildren(spawnFn, entitiesOut, width, height, backgroundRoom, perTerrain, world_gen_choices, is_adventuremode)
	...
	if children ~= nil then
		...
		for i,id in ipairs(children) do
			...
			for current_pos_idx = 1, #points_x do
				if perTerrain then --For use in bioms with mixed tile types (caves mostly)
...
function Node:PopulateExtra(world_gen_choices, spawnFn, data, is_adventuremode)
	...
	if world_gen_choices ~= nil and #data.idx_left >0 then
		...
		while idx< #data.idx_left do
			...
			if prefab ~= nil and AddEntityCheck(prefab, data.points_x[data.idx_left[idx]], data.points_y[data.idx_left[idx]], data.entitiesOut, self.data.terrain_contents.prefabspawnfn) then
				local prefab_data = {}
