...
local function MakeFollower(inst, leader)
    ...
    inst.components.floatable:UpdateAnimations("object_water", "idle_loop")
...
local function MakePlanted(inst)
    ...
    -- if we're being given, we don't want to be planted 
    inst.components.inventoryitem:SetOnPutInInventoryFn(function(inst) MakeItem(inst) end)

    inst.components.floatable:UpdateAnimations("object_water", "ground")
	inst:RemoveComponent("combat")
    inst:RemoveComponent("health")
    inst:RemoveComponent("burnable")
    inst:RemoveComponent("propagator")
...
local function defaultfn()
    ...
    MakeInventoryFloatable(inst, "object_water", "object")
