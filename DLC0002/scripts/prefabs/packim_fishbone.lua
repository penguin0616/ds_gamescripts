...
local function RebindPackim(inst, packim)
    packim = packim or TheSim:FindFirstEntityWithTag("chester")
    if packim then
...
local function RespawnPackim(inst)
    ...
    local packim = TheSim:FindFirstEntityWithTag("chester")
    if not packim then
...
return Prefab( "common/inventory/packim_fishbone", fn, assets)
