...
local function TransitionToNextLevel(inst, wilson)
	...
	local function onsave()
		scheduler:ExecuteInTime(110*FRAMES, function() 
			...
			inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_maxwelllaugh", "teleportato_laugh")
...
local function OnActivate(inst, doer)
	...
	if not inst.activatedonce then
		...
		inst.AnimState:PushAnimation("active_idle", true)
		inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_activate", "teleportato_activate")
		inst.SoundEmitter:KillSound("teleportato_idle")
		inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_activeidle_LP", "teleportato_active_idle")

		inst:DoTaskInTime(40*FRAMES, function()
			inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_activate_mouth", "teleportato_activatemouth")
		end)
		...
		local savemode = SaveGameIndex:GetCurrentMode(Settings.save_slot)
		if savemode == "shipwrecked" or savemode == "survival" then
...
local function PowerUp(inst)
	...
	inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_powerup", "teleportato_on")
	inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_idle_LP", "teleportato_idle")
...
local function ItemGet(inst, giver, item)
	if inst.collectedParts[item.prefab] ~= nil then
		...
		inst.SoundEmitter:PlaySound("dontstarve_DLC002/common/teleportato_shipwrecked/teleportato_addpart", "teleportato_addpart")
		TestForPowerUp(inst)
	end
end
