...
local function buildgem(colour)
    local fn = function()
        local Sparkle = function(inst)
            inst.components.floatable:UpdateAnimations(colour.."gem_idle_water_sparkle", colour.."gem_sparkle")

            if inst.components.floatable.onwater then
                inst.AnimState:PushAnimation(colour.."gem_idle_water")
            else
        ...
        MakeInventoryFloatable(inst, colour.."gem_idle_water", colour.."gem_idle")
