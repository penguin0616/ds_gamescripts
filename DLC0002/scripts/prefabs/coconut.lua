...
local function ondeploy (inst, pt) 
	...
	local ents = TheSim:FindEntities(pt.x,pt.y,pt.z, TUNING.PALMTREEGUARD_PINECONE_CHILL_RADIUS, {"leif"})
	
	local played_sound = false
	for k,v in pairs(ents) do
		
		local chill_chance = TUNING.PALMTREEGUARD_PINECONE_CHILL_CHANCE_FAR
		if distsq(pt, Vector3(v.Transform:GetWorldPosition())) < TUNING.PALMTREEGUARD_PINECONE_CHILL_CLOSE_RADIUS*TUNING.PALMTREEGUARD_PINECONE_CHILL_CLOSE_RADIUS then
			chill_chance = TUNING.PALMTREEGUARD_PINECONE_CHILL_CHANCE_CLOSE
		end
...
return Prefab( "common/inventory/coconut", raw, assets, prefabs),
    Prefab( "common/inventory/coconut_cooked", cooked, assets),
    Prefab( "common/inventory/coconut_halved", halved, assets),
	   MakePlacer( "common/coconut_placer", "coconut", "coconut", "planted" ) 
