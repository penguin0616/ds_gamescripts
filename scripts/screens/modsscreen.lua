...
local ModsScreen = Class(Screen, function(self, cb)
	...
	self.applybutton = self.mainmenu:AddItem(STRINGS.UI.MODSSCREEN.APPLY, function() self:Apply() end, Vector3(-170,-285,0))
	self.cancelbutton = self.mainmenu:AddItem(STRINGS.UI.MODSSCREEN.CANCEL, function() self:Cancel() end, Vector3(0,-285,0))
	...
	if PLATFORM ~= "WIN32_RAIL" then
		self.morebutton = self.mainmenu:AddItem(STRINGS.UI.MODSSCREEN.MOREMODS, function() self:MoreMods() end, Vector3(170,-285,0))
	end
