local assets =
{
    ...
    Asset("SOUND", "sound/ancientguardian_rework.fsb"),
}
...
local function ShouldSleep(inst)
    ...
    return homePos ~= nil
        and inst:GetDistanceSqToPoint(homePos) < SLEEP_DIST_FROMHOME_SQ
...
local function ShouldWake(inst)
    ...
    return (homePos ~= nil and
            inst:GetDistanceSqToPoint(homePos) >= SLEEP_DIST_FROMHOME_SQ)
        or BasicWakeCheck(inst)
end

local RETARGET_MUST_TAGS = { "hascombatcomponent" }
local RETARGET_CANT_TAGS = { "chess", "INLIMBO" }
local RETARGET_ONEOF_TAGS = { "character", "monster" }
local function Retarget(inst)
    local homePos = inst.components.knownlocations:GetLocation("home")
    return not (homePos ~= nil and
                inst:GetDistanceSqToPoint(homePos) >= MAX_CHASEAWAY_DIST_SQ)
        and FindEntity(
            inst,
...
local function KeepTarget(inst, target)
    ...
    local homePos = inst.components.knownlocations:GetLocation("home")
    return homePos ~= nil and inst:GetDistanceSqToPoint(homePos) < MAX_CHASEAWAY_DIST_SQ
end
...
local function cause_obstacle_quake(inst, other)
    if not other.components.timer or not other.components.timer:TimerExists("ram_quake") then
        ...
        if world and world:IsCave() then
            ShakeCamera("FULL", 2.5, .02, .5, inst, 40)
            world.components.quaker:MiniQuake(20, 20, 2.5, inst)
        end
...
local function onothercollide(inst, other)
    if not other:IsValid() or inst.recentlycharged[other] then
        ...
        if other:HasTag("quake_on_charge") then
            ...
            ShakeCamera("VERTICAL", .7, .02, 1.1, inst, 40)
            other:PushEvent("shake")
        else
            ShakeCamera("SIDE", .5, .05, .1, inst, 40)
        end
...
local function startobstacledrop(inst)
    local spawnlocation = inst.components.knownlocations:GetLocation("home")
    if spawnlocation then
        local dist = inst:GetDistanceSqToPoint(spawnlocation)

        if dist < 25*25 then
            local instpt = spawnlocation
            ...
            for i=1, math.random(3,4) do
                ...
                while testpt == nil and count < 8 do
                    ...
                    if not IsPassableAtPoint(testpt.x,0,testpt.z) then
                        testpt = nil
                    end
                    if testpt then
...
local function fn()
    ...
    MakeCharacterPhysics(inst, 1000, inst.physicsradius)
    inst.Physics:SetCapsule(1.75, 4) -- update inst.maxradius if you change this
    inst.physicsradiusoverride = 2.2
...
local function dospawnchest(inst, loading)
    ...
    --Set up chest loot
    chest.components.container:GiveItem(SpawnPrefab("cave_regenerator"))
...
local function bloodfn()
    ...
    inst:DoTaskInTime(21*FRAMES, SpawnTentacle)
