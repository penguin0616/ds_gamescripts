...
function PlayerController:GetActionButtonAction()
    ...
    if self:IsEnabled() and not (self.inst.sg:HasStateTag("working") or self.inst.sg:HasStateTag("doing")) then
        ...
        if pickup then -- and not has_active_item
            ...
            if notriding(self.inst) and (tool and tool.components.tool and pickup.components.workable and pickup.components.workable.workable and tool.components.tool:CanDoAction(pickup.components.workable.action)) then
