...
local states=
{
    ...
    State{
        ...
        onenter= function(inst)
            ...
            if inst.components.talkingbird then
                inst.components.talkingbird:Say(STRINGS.TALKINGBIRD.on_ground)
            else
        ...
        events=
        {
            EventHandler("animover", function(inst) 
                if not inst.components.talkingbird and math.random() < .5 then
                    inst.sg:GoToState("caw")
                else
                    inst.sg:GoToState("idle")
                end
    ...
    State{
        ...
        onenter= function(inst)
            ...
            if not inst.SoundEmitter:PlayingSound("flyin") then
        ...
        onupdate= function(inst)
            ...
            if pt.y <= .1 then
                ...
                if inst.components.periodicspawner and math.random() <= TUNING.CROW_LEAVINGS_CHANCE and inst.components.periodicspawner.onlanding then
    ...
    State{
        ...
        onenter = function(inst)
            ...
            if inst.sounds.takeoff2 then
                inst.SoundEmitter:PlaySound(inst.sounds.takeoff2)
            end
            
            if inst.components.periodicspawner and math.random() <= TUNING.CROW_LEAVINGS_CHANCE and not inst.components.periodicspawner.onlanding then
    ...
    State{
        ...
        events=
        {
            EventHandler("animover", function(inst) inst.sg:GoToState("stunned") end ),
        },        
    },    
