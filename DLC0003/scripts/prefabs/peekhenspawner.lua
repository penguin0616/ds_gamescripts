...
local function SpawnBuzzardShadow(inst)
	local buzzard = SpawnPrefab("circlingpeekhen")
...
local function LookForFood(inst)
    ...
    for k,v in pairs(ents) do
        if v and v:IsOnValidGround() and (v.components.edible and (v.components.edible.foodtype == "SEEDS" or v.components.edible.foodtype == "VEGGIE") and not v.components.inventoryitem:IsHeld())
    ...
    for k,v in pairs(ents) do 
    	if v and v:IsOnValidGround() and v:HasTag("eyebush") and not BuzzardNearFood(inst, v) and not v.buzzardHunted  then 
    		SpawnOnFood(inst, v)
    	end
    end 
end
...
local function circlingbuzzardfn()
    ...
    anim:SetBank("peekhen_shadow")
    anim:SetBuild("peekhen_shadow")
...
return Prefab( "badlands/objects/peekhenspawner", fn, assets, prefabs),
Prefab("badlands/objects/circlingpeekhen", circlingbuzzardfn, assets, prefabs)