...
local function startspawning(inst)
    if inst.components.spawner and inst.spawner then
...
local function testspawning(inst)
    if  GetClock():IsDay() or (GetClock():GetMoonPhase() == "new" and GetClock():IsNight()) and inst.spawner then                
        startspawning(inst)
    else
        stopspawning(inst)
    end
...
local function setupspawner(inst)
    ...
    inst:ListenForEvent( "nighttime", function() testspawning(inst) end, GetWorld())
    inst:ListenForEvent( "dusktime", function() testspawning(inst) end, GetWorld())
    inst:ListenForEvent( "daytime", function() testspawning(inst) end, GetWorld())    
end
...
local function makefn(build, stage, data)
    ...
    local function fn(Sim)
        ...
        inst:DoTaskInTime(0, function() if not inst.spawner then print("Cancel spawn") inst.components.spawner:CancelSpawning() else print("************* SPAWNER OK") end end )
