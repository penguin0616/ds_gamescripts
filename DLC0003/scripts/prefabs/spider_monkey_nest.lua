...
local function SetStage(inst, stage)
	if stage <= 1 then
		...
		if GetSeasonManager() and GetSeasonManager():IsGreenSeason() and inst.components.childspawner then
			inst.components.childspawner:SetMaxChildren(math.floor(TUNING.SPIDERDEN_SPIDERS[stage] * TUNING.SPRING_COMBAT_MOD))
...
--[[
    if not inst.components.health:IsDead() then
        ...
        if inst.components.childspawner then
            ...
            if GetSeasonManager() and GetSeasonManager():IsGreenSeason() then
            	num_to_release = math.floor(num_to_release * TUNING.SPRING_COMBAT_MOD)
            	num_warriors = math.floor(num_warriors * TUNING.SPRING_COMBAT_MOD)
            end
...
local function MakeSpiderDenFn(den_level)
	local spiderden_fn = function(Sim)
	    ...
	    inst:AddTag("hostile")
		inst:AddTag("spider_monkey_nest")
		inst:AddTag("hive")
...
return Prefab( "forest/monsters/spider_monkey_nest", MakeSpiderDenFn(1), assets, prefabs )
