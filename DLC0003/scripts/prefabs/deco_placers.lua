...
local function setScale(inst)
    inst.Transform:SetScale(1, .95, 1)
end
...
local function createmarkers(inst, pts)
    for i, subpt in ipairs(pts) do   
        ...
        if subpt.billboard then
            marker.AnimState:SetOrientation(ANIM_ORIENTATION.RotatingBillboard)  
            marker.Transform:SetRotation(-90)   
        end

        local color = Vector3(.25*0.1, .75*0.1, .25*0.1)
        marker.AnimState:SetAddColour(color.x, color.y, color.z, 1)
...
local function placeTestRug3Fn(inst, pt)
    return  placeTestRugFn(inst,pt,3)
end

local function placeTestRug25Fn(inst, pt)
    return  placeTestRugFn(inst,pt,2.5)
end
...
--    data.anim =  inst.animdata.anim .. "_front"
