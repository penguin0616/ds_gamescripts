...
local function onseasonchange_active(inst)
	if not inst.prevseason then
		inst.prevseason = GetSeasonManager():GetSeason()
		return
	end

	if GetSeasonManager():IsMildSeason() and inst.prevseason == SEASONS.DRY then
...
local function activefn(Sim)
	...
	inst:ListenForEvent("seasonChange", function() onseasonchange_active(inst) end, GetWorld() )
