...
local function ondonedrying(inst, product)
    if not inst:HasTag("burnt") then
        ...
        inst.onDoneDryingAnimOverCallback = function(inst)
	            inst:RemoveEventCallback("animover", inst.onDoneDryingAnimOverCallback)
	            setdone(inst, product)
	        end
		inst:ListenForEvent("animover", inst.onDoneDryingAnimOverCallback)
    end
end

local function onharvested(inst)
	if not inst:HasTag("burnt") then
	    if inst.onDoneDryingAnimOverCallback then
		    inst:RemoveEventCallback("animover",inst.onDoneDryingAnimOverCallback)
		    inst.onDoneDryingAnimOverCallback = nil
	    end
...
local function fn(Sim)
	...
	inst:ListenForEvent("rainstart", function(it) if not interiorcheck(inst) then PauseDrying(inst) end end, GetWorld())
