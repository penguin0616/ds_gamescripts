...
local function SetTraderPig(inst)
    inst:RemoveTag("werepig")
    inst:AddTag("trader")
    local brain = require "brains/pigtraderbrain"
    inst:SetBrain(brain)
    inst:SetStateGraph("SGpig")
    inst.AnimState:SetBuild(inst.build)
    
    inst.components.werebeast:SetOnNormalFn(SetTraderPig)
    inst.components.sleeper:SetResistance(3)
end
...
local function normal()
    ...
    inst.special_action = function (act)
        if inst.daily_gifting and inst.sg.currentstate.name ~= "daily_gift" then
...
return Prefab("common/characters/pigman", normal, assets, prefabs),
       ...
       Prefab("common/character/pigtrader", trader, assets, prefabs) 