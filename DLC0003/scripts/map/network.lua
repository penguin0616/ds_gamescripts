...
Graph = Class(function(self, id, args)   
	...
	self.treasures = args.treasures
	self.random_treasures = args.random_treasures
...
function Graph:LockGraph(id, left_exit_node, right_exit_node, lock)
	...
	self.exit_edges[id] = Edge(id, left_exit_node, right_exit_node, lock, {colour = {r=255, g=255, b=255, a=255}})
...
function Graph:PopulateVoronoi(spawnFN, entities, width, height, world_gen_choices, prefab, is_adventuremode)
	local nodes = self:GetNodes(false)
	--print(self.id.." Populating "..GetTableSize(nodes).." nodes...")	
	for k,node in pairs(nodes) do
		node:PopulateVoronoi(spawnFN, entities, width, height, world_gen_choices, prefab, is_adventuremode)
		...
		node:PopulateChildren(spawnFN, entities, width, height, backgroundRoom, perTerrain, world_gen_choices, prefab, is_adventuremode)
	end 
	for k,child in pairs(self:GetChildren()) do
		child:PopulateVoronoi(spawnFN, entities, width, height, world_gen_choices, prefab, is_adventuremode)
	end
end
...
function Graph:GlobalPrePopulate(entities, width, height)
	...
	--self.MIN_WORMHOLE_ID = WorldGenPlaceTreasures(self:GetChildren(), entities, width, height, self.MIN_WORMHOLE_ID)
end
...
function Graph:GlobalPostPopulate(entities, width, height)
	...
	-- Spawn bermuda triangles
	self:SwapOutBermudaTriangleMarkers(entities, width, height)
...
function Graph:SwapOutPrefabMarkers(entities, width, height, prefab, prefab_marker, max_pairs, min_distance)
	print("SwapOutPrefabMarkers " .. prefab_marker .. " -> " .. prefab)
	if entities[prefab_marker] ~= nil then
		print(#entities[prefab_marker] .. " " .. prefab_marker)
		...
		local pair = 0
		local min_distsq = min_distance * min_distance
		local is_farenough = function( marker1, marker2, distsq)
			--[[
			for i = 1, #entities[prefab_marker], 1 do
				local marker2 = entities[prefab_marker][i]
				if marker2 ~= nil and marker ~= marker2 then
					local diffx, diffz = marker2.x - marker.x, marker2.z - marker.z
		...
		for i = 1, #entities[prefab_marker], 1 do
			local firstMarkerData = entities[prefab_marker][i]
			if firstMarkerData ~= nil then
				for j = 1, #entities[prefab_marker], 1 do
					local secondMarkerData = entities[prefab_marker][j]
					if secondMarkerData ~= nil and i ~= j and is_farenough(firstMarkerData, secondMarkerData, min_distsq) then
						local diffx, diffz = secondMarkerData.x - firstMarkerData.x, secondMarkerData.z - firstMarkerData.z
						local mag = diffx * diffx + diffz * diffz
						--print(string.format("%d %d: dist %f, min %f\n", i, j, math.sqrt(mag), min_distance))
						--if mag >= min_distance * min_distance then
							...
							table.insert(entities[prefab], firstMarkerData)
							table.insert(entities[prefab], secondMarkerData)

							entities[prefab_marker][i] = nil
							entities[prefab_marker][j] = nil
							break
						--end
					end
				end
		...
		print(pair .. " " .. prefab .. " pairs placed.")

		self.MIN_WORMHOLE_ID = id
		entities[prefab_marker] = nil
	end	
end

function Graph:SwapOutWormholeMarkers(entities, width, height)
	self:SwapOutPrefabMarkers(entities, width, height, "wormhole", "wormhole_MARKER")
end

function Graph:GenerateBermudaTriangleMarkers(entities, width, height)
	-- body
	...
	local function checkTriangle(tile, x, y)
		if  tile ~= GROUND.OCEAN_DEEP then 
			return false 
		end 
		for i = 1, #points, 1 do 
			...
			if dsq < minDistance * minDistance then 
				return false 
			end 
		end 
		return true 
	...
	for i = 1, numTriangles, 1 do 
		local pointsX, pointsY = GetRandomWaterPoints(checkTriangle, width, height, TUNING.MAPWRAPPER_WARN_RANGE + 10, 1, true)
		if pointsX[1] then 
			table.insert(points, {x = pointsX[1], y = pointsY[1]})
		end 
	end 

	for i = 1, #points, 1 do 
		local entData = {}
		local x = (points[i].x - width/2.0)*TILE_SCALE
		local y = (points[i].y - height/2.0)*TILE_SCALE
		entData.x = x
		entData.z = y
...
function Graph:SwapOutBermudaTriangleMarkers(entities, width, height)
	self:GenerateBermudaTriangleMarkers(entities, width, height)
	self:SwapOutPrefabMarkers(entities, width, height, "bermudatriangle", "bermudatriangle_MARKER")
end
