...
function InvSlot:Click(stack_mod)
	...
	if container_item and container_item.onshelf then
		-- not doing that on a shelf, sorry
		return
	end

    if container_item and container_item.components.equippable and container_item.components.equippable.un_unequipable then
        return
    end    
    ...
    if character and inventory and container_item then
        ...
        for k,v in pairs(inventory.opencontainers) do
            if k ~= container.inst and (not dest_inst or not k.components.equippable) then
                --TODO: BDOIG QUICK FIX FOR NOW! - revert change once "equipper" component is done so some creatures don't have both container and inventory components.
                local dest = (k.components.inventory and k.components.inventory.acceptitems and k.components.inventory) or k.components.container
                if dest then
        ...
        if dest_inst then  
            --TODO: BDOIG QUICK FIX FOR NOW! - revert change once "equipper" component is done so some creatures don't have both container and inventory components.
            local dest = (dest_inst.components.inventory and dest_inst.components.inventory.acceptitems and dest_inst.components.inventory) or dest_inst.components.container
            if dest then
                local item = nil
				...
				-- don't drop on fail, since the item may transform on drop (eg a jellyfish)
                if not dest:GiveItem(item, nil, nil, nil, true) then
