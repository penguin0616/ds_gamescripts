...
local PLAY_POST_WORLDGEN_MOVIE = false
...
local function LoadAssets(asset_set)
	...
	AddToRecipePrefabs(Porkland_Recipes, RECIPE_PREFABS)
...
local function HandleDeathCleanup(wilson, data)
    ...
    if SaveGameIndex:GetCurrentMode() == "survival" or SaveGameIndex:GetCurrentMode() == "cave" or SaveGameIndex:GetCurrentMode() == "volcano" or SaveGameIndex:GetCurrentMode() == "shipwrecked" or SaveGameIndex:GetCurrentMode() == "porkland" then
...
POPULATING = false
function PopulateWorld(savedata, profile, playercharacter, playersavedataoverride, playerevent)
    POPULATING = true
    TheSystemService:SetStalling(true)
    ...
    if savedata then
        ...
        local travel_origin = SaveGameIndex:GetOriginOfTravel()
		...
		elseif travel_direction == "shipwrecked" and travel_origin == "survival" and savedata.ents["portal_shipwrecked"] ~= nil then
			-- Moving from ROG/Vanilla to a recently generated world
			spawn_ent = savedata.ents["portal_shipwrecked"][1]
		elseif (travel_direction == "shipwrecked" or travel_direction == "survival" or travel_direction == "porkland") then
		...
		elseif savedata.map.prefab == "porkland" then
			world = SpawnPrefab("porkland")
		else
		...
		local worldVersion = tonumber(savedata.meta.build_version)
		local movedInteriorSpawnOrigin = savedata.map and savedata.map.persistdata and savedata.map.persistdata.movedInteriorSpawnOrigin
		local shouldMoveInteriorSpawnOrigin = (worldVersion <= 343551) and not movedInteriorSpawnOrigin

        wilson = SpawnPrefab(playercharacter)
        assert(wilson, "could not spawn player character")
        wilson:SetProfile(Profile)

		local px,py,pz = spawnpoint.x, spawnpoint.y, spawnpoint.z
		if shouldMoveInteriorSpawnOrigin then
			...
			px, py, pz = InteriorSpawner.FixForSpawnOriginMigration(spawnpoint.x, spawnpoint.y, spawnpoint.z,"player")
			InteriorSpawner.deltaForSpawnOriginMigration = Vector3(px - spawnpoint.x, py - spawnpoint.y, pz - spawnpoint.z)
			GetWorld().movedInteriorSpawnOrigin = true
			if savedata.map and savedata.map.persistdata then
				savedata.map.persistdata.movedInteriorSpawnOrigin = true
			end
		end
        wilson.Transform:SetPosition(px, py, pz)
        ...
        if ground then
	        ...
	        elseif savedata.map.prefab == "shipwrecked" then
	        	...
	        	ground.Map:SetOneWayEdgeCollision(false)
	        	TheFrontEnd:GetGraphicsOptions():DisableStencil()
	        	TheFrontEnd:GetGraphicsOptions():DisableLightMapComponent()
	        ...
	        elseif savedata.map.prefab == "porkland" then
	        	...
	        	TheFrontEnd:GetGraphicsOptions():DisableStencil()
	        	TheFrontEnd:GetGraphicsOptions():DisableLightMapComponent()
	        	ground.Map:Finalize(0, COLLISION_TYPE.POND)
	        else
			...
			wilson:ListenForEvent( "changearea", function(inst, area)    ground:PushEvent("changearea", area)     end)
...
--					end
        ...
        SaveGameIndex:ClearCurrentWorldEntrances()
		...
		-- did we come in through sea or otherworthy?
		if playerevent then
			local entrance = SaveGameIndex:GetWorldEntranceForCurrentWorld(playerevent)
			local entid = entrance and tonumber(string.split(entrance, ":")[2])
			local ent = Ents[entid]
			if ent then
				local was_invincible = wilson.components.health:IsInvincible()
				ent:UpdateIsInInterior()
				wilson:Teleport(ent, true)
				GetInteriorSpawner():FadeInFinished(was_invincible)
			end
		end
		...
		if GetClock() ~= nil and SaveGameIndex:GetCurrentMode() ~= "adventure" then
			...
			if clock_data ~= nil then -- we just travelled between worlds!
				...
				local direction_of_travel = SaveGameIndex:GetDirectionOfTravel()
				if (savedata.meta.freshly_merged == true)
					and (direction_of_travel == "shipwrecked" or direction_of_travel == "survival" or direction_of_travel == "porkland") then
		...
		if GetWorld().ConnectIslands then
			GetWorld():ConnectIslands()
		end
    else
...
function DoInitGame(playercharacter, savedata, profile, next_world_playerdata, fast, playerevent)	
    ...
    local wilson = PopulateWorld(savedata, profile, playercharacter, next_world_playerdata, playerevent)
    if wilson then
	...
	if not TheFrontEnd:IsDisplayingError() then
	    ...
	    if fast then
			...
			elseif GetWorld():IsCave() then
				...
				GetPlayer().HUD:Show()
			elseif Settings.playeranim == "wakeup" or (playercharacter == "waxwell" and SaveGameIndex:GetCurrentMode() ~= "shipwrecked" and SaveGameIndex:GetCurrentMode() ~= "porkland") or savedata.map.nomaxwell then
			...
			elseif (GetClock().numcycles == 0 and GetClock():GetNormTime() == 0) or Settings.maxwell ~= nil then
				...
				if SKIP_MAXWELL_INTRO ~= nil and not SKIP_MAXWELL_INTRO then
					if savedata.map.prefab == "porkland" then
						local max = SpawnPrefab("porkland_intro")
						local speechName = "NULL_SPEECH"
						if GetPlayer().prefab == "wormwood" then

							local x,y,z  = GetPlayer().Transform:GetWorldPosition()
							local ents = TheSim:FindEntities(x,y,z, 20,{"porklandintro"})
							...
							x = math.floor(x) +.5
							z = math.floor(z) +.5
							local wall1 = SpawnPrefab("wall_pig_ruins")
							wall1.Transform:SetPosition(x-3,0,z-3)
							local wall2 = SpawnPrefab("wall_pig_ruins")
							wall2.Transform:SetPosition(x+3,0,z+3)	
							local wall3 = SpawnPrefab("wall_pig_ruins")
							wall3.Transform:SetPosition(x+3,0,z-3)	
							local wall4 = SpawnPrefab("wall_pig_ruins_repaired")
							wall4.Transform:SetPosition(x-3,0,z+3)	

							local rock1 = SpawnPrefab("rocks")
							rock1.Transform:SetPosition(x+2.3,0,z-1.3)	
							local rock2 = SpawnPrefab("rocks")
							rock2.Transform:SetPosition(x-1.5,0,z-2.4)	
							local rock3 = SpawnPrefab("rocks")
							rock3.Transform:SetPosition(x+1.1,0,z+2.1)	

							
							speechName = "PORKLAND_2"
						else
							speechName = "PORKLAND_1"
						end
						...
						if PLAY_POST_WORLDGEN_MOVIE then
							playingMovie = true
						end
					elseif savedata.map.prefab == "shipwrecked" then
			...
			if playingMovie then
				...
				TheFrontEnd:Fade(true, 0.5, function()
...
function TravelBetweenWorlds(targetmode, playerevent, waittime, dropitems_tag, customoptions, mergefromslot)
	...
	local function onmerged()
		-- kick us out of our current interior, so that when we return to this world from another route we won't be stuck in interior mode
		GetInteriorSpawner():LeaveCurrentInterior()
		SaveGameIndex:SaveCurrent(function() SaveGameIndex:EnterWorld(targetmode, onentered, nil, nil, nil, customoptions) end, targetmode)
	end

	local function onstartnextmode()
		if mergefromslot ~= nil then
			local dlc = targetmode == "porkland" and {REIGN_OF_GIANTS = false, CAPY_DLC = false, PORKLAND_DLC = true} or nil
...
local function DoLoadWorld(saveslot, playerdataoverride, playerevent)
	local function onload(savedata)
		...
		DoInitGame(SaveGameIndex:GetSlotCharacter(saveslot), savedata, Profile, playerdataoverride, nil, playerevent)
	end
	SaveGameIndex:GetSaveData(saveslot, SaveGameIndex:GetCurrentMode(saveslot), onload)
end

local function DoGenerateWorld(saveslot, type_override, playerevent)
	local function onComplete(savedata )
		...
		local function onsaved()
			...
			if success then
				LoadAssets("BACKEND")
				DoInitGame(SaveGameIndex:GetSlotCharacter(saveslot), world_table, Profile, SaveGameIndex:GetPlayerData(saveslot), nil, playerevent)
			end
		end
...
local function ShouldSkipMainScreen()
	local slotnum = TheSim:GetSetting("misc", "skip_to_slot")

    if slotnum then
    	slotnum = tonumber(slotnum)
    	if slotnum < 1 or slotnum > 5 then
    		return nil
    	end
    	return slotnum
    end
...
local function SkipMainScreen(slotnum)
    ...
    StartNextInstance({reset_action=RESET_ACTION.LOAD_SLOT, save_slot = slotnum})
end

local function MainScreenFlow()
	local skip_slot = ShouldSkipMainScreen()
	if skip_slot and Settings.last_reset_action ~= RESET_ACTION.LOAD_SLOT then
		SkipMainScreen(skip_slot)
	else
		LoadAssets("FRONTEND")
...
local function LoadSlot(slot, playerevent)
	TheFrontEnd:ClearScreens()
	...
	if SaveGameIndex:HasWorld(slot, SaveGameIndex:GetCurrentMode(slot)) then
		...
		LoadAssets("BACKEND")
   		DoLoadWorld(slot, SaveGameIndex:GetModeData(slot, SaveGameIndex:GetCurrentMode(slot)).playerdata, playerevent)
	else
		print("Load Slot: Has no World")
		if (SaveGameIndex:GetCurrentMode(slot) == "survival" or SaveGameIndex:GetCurrentMode(slot) == "shipwrecked" or SaveGameIndex:GetCurrentMode(slot) == "porkland") and SaveGameIndex:IsContinuePending(slot) then
			print("Load Slot: ... but continue pending")
			
			local function onsave()
				DoGenerateWorld(slot, nil, playerevent)
			end
	...
	if Settings.reset_action then
		...
		elseif Settings.reset_action == RESET_ACTION.LOAD_SLOT then
			if not SaveGameIndex:GetCurrentMode(Settings.save_slot) then
				...
				LoadSlot(Settings.save_slot, Settings.playerevent)
			end
