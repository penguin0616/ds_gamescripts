...
local rcol = RESOLUTION_X/2 -200 -- 150?
local lcol = -RESOLUTION_X/2 +200 -- 150?
...
function MainScreen:DoInit( )
    ...
    self.shield:GetAnimState():SetBank("title_hamlet")
    self.shield:GetAnimState():SetBuild("hamlet_title_shield")
	...
	if EARLYACCESS_ON == true then
		self.watermark = self.bg:AddChild(Text(UIFONT, 25))
		self.watermark:SetPosition(0, 330)
		self.watermark:SetRegionSize( 400, 44 )
		self.watermark:SetHAlign(ANCHOR_MIDDLE)
		self.watermark:SetVAlign(ANCHOR_TOP)
	...
	self.updatename:SetString(STRINGS.UI.MAINSCREEN.PORKLAND_UPDATENAME .. suffix)
    self.updatename:SetColour(0,0,0,1)

--[[
	self.EA = self.shield:AddChild(Image("images/hud_porkland.xml", "early_access.tex"))
    self.EA:SetVRegPoint(ANCHOR_MIDDLE)
    self.EA:SetHRegPoint(ANCHOR_MIDDLE)
    self.EA:SetPosition(0, 110, 0)
    self.EA:SetScale(.8)
]]
	...
	self.submenu:SetPosition(80, -275, 0)
	self.submenu:SetScale(.8)

	if not IsDLCInstalled(REIGN_OF_GIANTS) and PLATFORM == "WIN32_STEAM" then		
		self.DLCUpgrade = self.left_col:AddChild(DLCUpgrade())
	    self.DLCUpgrade:SetScale(.7)
	    self.DLCUpgrade:SetPosition(-50, -215, 0)
	else
		--[[
		self.wilson = self.left_col:AddChild(UIAnim())
	    self.wilson:GetAnimState():SetBank("corner_dude_pork")
	    self.wilson:GetAnimState():SetBuild("corner_dude_pork")
	    self.wilson:GetAnimState():PlayAnimation("idle",true)
	   	self.wilson:SetPosition(-75,-370,0)
		]]	
		self.wilson = self.left_col:AddChild(UIAnim())
	    self.wilson:GetAnimState():SetBank("corner_dude_townspig")
	    self.wilson:GetAnimState():SetBuild("corner_dude_townspig")
	    self.wilson:GetAnimState():PlayAnimation("idle",true)
	    self.wilson:SetPosition(-30,-450,0)
	    self.wilson.getanimtime = function() return math.random(2,5) end
	    self.wilson.animtimer = self.wilson.getanimtime()		
	end
	...
	self.motd:SetPosition(-55, RESOLUTION_Y/2-200, 0)
	--self.motd:Hide()
	self.motdbg = self.motd:AddChild( Image( "images/globalpanels.xml", "panel.tex" ) )
	self.motdbg:SetScale(.68*.9,.68,.68)
	...
	self.motd.motdimage.image:SetScale( 1, 1, 1 )
    ...
    self.motd.motdimage.OnGainFocus =
		function()
			...
			self.motd:SetScale(0.84,0.84,0.84)
		end
	self.motd.motdimage.OnLoseFocus =
		function()
    		self.motd.motdimage.image:SetTexture(self.motd.motdimage.atlas, self.motd.motdimage.image_normal)
			self.motd:SetScale(.81,.81,.81)
		end
...
function MainScreen:DoOptionsMenu()
    ...
    if HAVE_SUBMIT_ERRORREPORT then
	  	table.insert( menu_items, {text=STRINGS.UI.MAINSCREEN.BUGREPORT, cb= function() self:BugReport() end})
    end
	...
	if BRANCH ~= "release" then
		self:ShowMenu(menu_items, 80, -85)
	else
		self:ShowMenu(menu_items, 80, -50)
	end	
end
...
function MainScreen:CheatMenu()
	...
	self:ShowMenu(menu_items, 80, 107)
end
...
function MainScreen:MainMenu()
	...
	self:ShowMenu(menu_items, 80, 35)
	self.mainmenu = true
	--self.inst:DoTaskInTime(0,function() self:HamletBeta() end)
end

function MainScreen:OnBecomeActive()
    MainScreen._base.OnBecomeActive(self)    
	self.menu:SetFocus()
	self:NONPC_DataCollectionResetPopup()
end
...
function MainScreen:OnUpdate(dt)
    if TheSim:ShouldPlayIntroMovie() then
        local moviename = PLATFORM == "PS4" and "movies/forbidden_knowledge.mp4" or "movies/intro.ogv"
        TheFrontEnd:PushScreen( MovieDialog(moviename, function() TheFrontEnd:GetSound():PlaySound("dontstarve_DLC003/music/theme","FEMusic") end ) )
	...
	if self.wilson then
		if self.wilson.animtimer and self.wilson.animtimer > 0 then
			self.wilson.animtimer = self.wilson.animtimer - dt
		else
			local ANIMS = {
				"idle_happy",
				"emote_hat",
			...
			self.wilson:GetAnimState():PlayAnimation(ANIMS[math.random(1,#ANIMS)])
			self.wilson:GetAnimState():PushAnimation("idle",true)
			self.wilson.animtimer = self.wilson.getanimtime()
		end
	end
...
function MainScreen:SetMOTD(str, cache)
	...
	if status and motd then
		...
		if PLATFORM == "WIN32_STEAM" or PLATFORM == "LINUX_STEAM" or PLATFORM == "OSX_STEAM" then
			platform_motd = motd.hamletsteam
		else
			platform_motd = motd.hamletstandalone
		end
...
function MainScreen:NONPC_DataCollectionResetPopup()

	local nonWindowsPlatform = not (PLATFORM == "WIN32_STEAM" or PLATFORM == "WIN32")
	local agreementOnStartup = TheSim:AgreementsSettingsExistedOnStartup()
	local firstTimeAfterInstall = TheSim:FirstTimePlayingAfterInstall()

	if nonWindowsPlatform and not agreementOnStartup and not firstTimeAfterInstall and not self.alreadyViewedDataCollectionResetPopup then
		self.alreadyViewedDataCollectionResetPopup = true

		TheFrontEnd:PushScreen(
			BigerPopupDialogScreen( STRINGS.UI.MAINSCREEN.DATACOLLECTION_NOTICE, STRINGS.UI.MAINSCREEN.DATACOLLECTION_BODY,
			  {
			  	{
			  		text = STRINGS.UI.MAINSCREEN.DATACOLLECTION_DISABLE,
			  		cb = function()
			  			Profile:SetAgreementsSetting(false)
			  			TheSim:EnableUserDataCollection(false)
			  			Profile:Save(function() end)
			  			TheFrontEnd:PopScreen()
					end
				},
			  	{
			  		text = STRINGS.UI.MAINSCREEN.DATACOLLECTION_CLOSE,
			  		cb = function()			
			  			TheFrontEnd:PopScreen()
					end
				},
			  	{
			  		text = STRINGS.UI.MAINSCREEN.DATACOLLECTION_VIEW,
			  		cb = function()
...
function MainScreen:HamletBeta()

	local firstTimeAfterInstall = TheSim:FirstTimePlayingAfterInstall()

	if not self.alreadyViewedBetaPopup and not Profile:GetHamletWarned() and EARLYACCESS_ON == true then
		self.alreadyViewedBetaPopup = true

		Profile:SetHamletWarned(true)
		Profile:Save(function() end)
