...
local CustomizationScreen = Class(Screen, function(self, profile, cb, defaults, ROGEnabled, merge_options, world)
	...
	local world_files = self:GetWorldFiles()
	customise = require(world_files[world])
	...
	for k,v in pairs(customise.GROUP) do
		...
		for kk, vv in pairs(nextgroup.items) do
			...
			table.insert(options, {name = nextitemname, image = nextitem.image, options = nextitem.desc or nextgroup.desc, default = nextitem.value, group = nextgroupname, atlas = nextitem.atlas})
			numitem = numitem + 1
		end
		numgroup = numgroup + 1
		numitem = 1
	...
	print(CAPY_DLC, REIGN_OF_GIANTS, PORKLAND_DLC )
	SetManualBGColor(self.bg, world)
	...
	local level_table = 
	{
		MAIN_GAME = levels.sandbox_levels,
		REIGN_OF_GIANTS = levels.sandbox_levels,
		CAPY_DLC = levels.shipwrecked_levels,
		PORKLAND_DLC = levels.porkland_levels
	}

	local levelpresets = level_table[world]

	-- print ("########", world, CAPY_DLC)
	-- if world == CAPY_DLC then
	-- 	levelpresets = levels.shipwrecked_levels
	-- elseif world == PORKLAND_DLC then
	-- 	levelpresets = levels.porkland_levels
	-- end
	...
	-- 	end
	-- else
	-- 	if IsDLCEnabled(CAPY_DLC) and world == CAPY_DLC then
	-- 		levelpresets = levels.shipwrecked_levels
	-- 	end
	-- end


	for i, level in pairs(levelpresets) do
		table.insert(self.presets, {text=level.name, data=level.id, desc = level.desc, overrides = level.overrides})
	end

	self.presets_level_size = #levelpresets
...
function CustomizationScreen:GetWorldFiles()
	...
	elseif IsDLCEnabled(CAPY_DLC) then
		return {
			MAIN_GAME = "map/customise_base",
			REIGN_OF_GIANTS = "map/customise_rog",
			CAPY_DLC = "map/customise",
		}
	else
		return {
			MAIN_GAME = "map/customise",
			REIGN_OF_GIANTS = "map/customise",
		}
	end
end
...
function CustomizationScreen:RefreshOptions()
	...
	for k = 1, per_side*2 do
		...
		if options[idx] then
			local nameuppercase = string.upper(options[idx].name)
			local tooltip = STRINGS.NAMES[nameuppercase] or STRINGS.UI.CUSTOMIZATIONSCREEN.NAMES[nameuppercase] or options[idx].name
		    ...
		    image:SetTooltip(tooltip) --(options[idx].name)
...
function CustomizationScreen:VerifyValidSeasonSettings()
	if self.world == "PORKLAND_DLC" then
		local temperate = self:GetValueForOption("temperate_season")
		local humid = self:GetValueForOption("humid_season")
		local lush = self:GetValueForOption("lush_season")	

		if temperate == "noseason" and lush == "noseason" and humid == "noseason" then
			return false
		end	
	elseif self.world == "CAPY_DLC" then
		local mild = self:GetValueForOption("mild_season")
		local wet = self:GetValueForOption("wet_season")
		local green = self:GetValueForOption("green_season")
		local dry = self:GetValueForOption("dry_season")
		if mild == "noseason" and wet == "noseason" and green == "noseason" and dry == "noseason" then
			return false
		end
	else
...
function CustomizationScreen:SavePreset()
	...
	local function AddPreset(index, presetdata)
		local base = self.presetspinner:GetSelectedIndex() <= self.presets_level_size and self.presetspinner:GetSelected().data or self.presetspinner:GetSelected().basepreset
		...
		self.presets[index + self.presets_level_size] = preset
		self.presetspinner:SetOptions(self.presets)
		self.presetspinner:SetSelectedIndex(index + self.presets_level_size)
...
function CustomizationScreen:Apply()
	...
	local function collectCustomPresetOptions()
		...
		if self.presetspinner:GetSelectedIndex() > self.presets_level_size then
	...
	if self.ROGEnabled then
		self.options.ROGEnabled = self.ROGEnabled
	end
