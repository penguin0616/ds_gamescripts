...
function Quaker_Interior:OnProgress()
	self.noserial = true
end
...
function Quaker_Interior:GetMammalSpawnPoint()
	...
	if not interiorspawner:IsInInterior() then
		return nil
	end

	local interior = interiorspawner.current_interior
	local basept = interiorspawner:getSpawnOrigin()
...
function Quaker_Interior:StartQuake()
...
function Quaker_Interior:EndQuake()
	self.quake = false
...
function Quaker_Interior:IsQuaking()
	return self.quake
end
...
function Quaker_Interior:ForceQuake(level)
	--print("FORCE QUAKE")
...
local function grounddetection_update(inst)
	...
	if pt.y <= .2 then
		...
		local quaker = GetWorld().components.quaker_interior
