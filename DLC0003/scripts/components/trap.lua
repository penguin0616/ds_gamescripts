...
-- Drop our loot (for when we're picked up without being harvested)
function Trap:DropLoot()
	if self.issprung then
	    local timeintrap = self.inst.components.timer:GetTimeElapsed("foodspoil") or 0
	    if self.lootprefabs and GetPlayer().components.inventory then
...
function Trap:cantrap()
    local cantrap = true 
    local onwater = self.inst:GetIsOnWater(self.inst:GetPosition():Get())

    if (self.water and not onwater) or (not self.water and onwater) then
        cantrap = false            
    end   
...
function Trap:OnUpdate(dt)
    if self.isset and self:cantrap() then
...
function Trap:DoSpring()
    ...
    if self.bait and self.bait:IsValid() then
        ...
        local ents = TheSim:FindEntities(x,y,z, 2, nil, {"FX", "DECOR"})
