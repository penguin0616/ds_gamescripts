...
function Eater:SetElemental(human)
    ...
    if human then
        self.ablefoods = { "MEAT", "VEGGIE", "INSECT", "SEEDS", "GENERIC", "ELEMENTAL", "GOLDDUST" }
    else
        self.ablefoods = { "ELEMENTAL", "GOLDDUST" }
    end
end
...
function Eater:Eat( food )
    if self:CanEat(food) then
        ...
        local naughtyvalue = food.components.edible:GetNaughtiness(self.inst)
