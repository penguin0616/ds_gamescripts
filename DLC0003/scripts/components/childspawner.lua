...
function ChildSpawner:StartSpawning(timetonextspawn)
...
function ChildSpawner:GetDebugString()
	...
	if self.timetonextspawn then
		str = str..string.format(" timetonextspawn var: %2.2f  maxChildren: %d",self.timetonextspawn,self.maxchildren)
	end
	return str
end
...
function ChildSpawner:TakeOwnership(child)
    ...
    if not child.components.homeseeker or child.components.homeseeker.home ~= self.inst then
...
function ChildSpawner:ReplaceChild(oldEnt, newEnt)
...
function ChildSpawner:SpawnChild(target, prefab, radius)
	...
	if not self.nooffset then
		local offset = nil
		if self.spawnonwater then
			local radius = self.inst.Physics:GetRadius()
			if self.spawnonwateroffset then
				radius = self.spawnonwateroffset + radius
			end
			local tries = 0
			while tries < 20 and offset == nil do
				local check_angle = math.random() * 360
				offset = Vector3(radius * math.cos( check_angle ), 0, -radius * math.sin( check_angle ))
				tries = tries + 1
				if not self.inst:GetIsOnWater(pos.x+offset.x, 0 ,pos.z+offset.z)  then
					print("FAILED NOT ON WATER",tries)
					offset = nil
				end
			end
		else
			offset = FindWalkableOffset(pos, start_angle, rad, 8, false)			
		end

		if offset == nil then
			-- failed to find a spawn location
			return nil, true
		end		
		pos = pos + offset
	end
...
function ChildSpawner:GoHome( child )
    if self.childrenoutside[child] then
        ...
        --self.numchildrenoutside = self.numchildrenoutside - 1   handled by the "onremoved" function of the child that triggers the on killed function here
        self:AddChildrenInside(1)
        return true
    end
end
...
function ChildSpawner:ReleaseAllChildren(target, prefab)
    while self:CanSpawn() do
        local inst, nospacefound = self:SpawnChild(target, prefab)
		if nospacefound then
			break
		end
    end
end
...
function ChildSpawner:AddChildrenInside(count)
    ...
    if self.maxchildren and not self.allowmorethanmaxchildren then
