...
function Quaker:SpawnDebris(spawn_point, in_interior)
    local prefab = self:GetDebris()

    if in_interior and math.random() < 0.75 and not (prefab == "mole" or prefab == "rabbit") then
        -- Simulate debris break.
        return
    end
    ...
    if prefab then
        ...
        spawn_point.y = in_interior and 0 or 35
...
function Quaker:MiniQuake(rad, num, duration, target)
    ...
    for i=1,num do
        ...
        self.inst:DoTaskInTime(time, function()
            ...
            local char_pos = in_interior and Vector3(interiorSpawner:GetInteriorEntryPosition()) or target:GetPosition()
