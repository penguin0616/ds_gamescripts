...
function MinionSpawner:OnLostMinion(minion)
	if minion then
		self.inst:DoTaskInTime(self.addpositiondelay, function() self:AddPosition(minion.minionnumber) end)
...
function MinionSpawner:MakeSpawnLocations()
	...
	for k,v in pairs(positions) do
		...
		#TheSim:FindEntities(try_pos.x, try_pos.y, try_pos.z, 1, nil, {"FX", "NOCLICK", "DECOR", "INLIMBO"}) <= 0 then
...
function MinionSpawner:SpawnNewMinion(force)
