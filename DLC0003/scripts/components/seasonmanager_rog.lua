...
function SeasonManager:OnUpdate( dt )	
	...
	--print ("Current season " .. self.current_season)
	...
	if not self.incaves and not self.ininterior then
		if TheCamera.interior then  -- could also be GetPlayer():CheckIsInInterior()
			...
			if self.pollen then 
				self.pollen.particles_per_tick = 0
			end
...
function SeasonManager:IsGreenSeason()
	return false
end
