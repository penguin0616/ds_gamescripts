...
local function DoneBurning(inst)
    ...
    if burnable then
        if burnable.canActuallyBurnFunction then
            local canburn, delay = burnable.canActuallyBurnFunction(inst)
            if not canburn then
                -- if no delay is returned try next frame
                inst:DoTaskInTime(delay or 0, DoneBurning)
                return
            end
        end
        ...
        if GetPlayer():HasTag("plantkin") and inst:HasTag("plant") then
            if inst:IsNear(GetPlayer(), 20) then
...
function Burnable:OnSave()
    if self.disabled then
        return {disabled = self.disabled}
    end
end

function Burnable:OnLoad(data)
    if data.disabled then
        self.disabled = data.disabled
    end
end

function Burnable:SetCanActuallyBurnFunction(fn)
	self.canActuallyBurnFunction = fn
end
...
function Burnable:RestoreInventoryItemData()
    if self.inst.inventoryitemdata and not self.inst.inventoryitem then
        ...
        if self.inst.inventoryitemdata["foleysound"] ~= nil then self.inst.components.inventoryitem.foleysound = self.inst.inventoryitemdata["foleysound"] end
        if self.inst.inventoryitemdata["onputininventoryfn"] ~= nil then self.inst.components.inventoryitem.onputininventoryfn = self.inst.inventoryitemdata["onputininventoryfn"] end
        if self.inst.inventoryitemdata["cangoincontainer"] ~= nil then self.inst.components.inventoryitem.cangoincontainer = self.inst.inventoryitemdata["cangoincontainer"] end
        if self.inst.inventoryitemdata["nobounce"] ~= nil then self.inst.components.inventoryitem.nobounce = self.inst.inventoryitemdata["nobounce"] end
        if self.inst.inventoryitemdata["canbepickedup"] ~= nil then self.inst.components.inventoryitem.canbepickedup = self.inst.inventoryitemdata["canbepickedup"] end
        if self.inst.inventoryitemdata["imagename"] ~= nil then 
            self.inst.components.inventoryitem.imagename = self.inst.inventoryitemdata["imagename"] 
            self.inst:PushEvent("imagechange")
        end
        if self.inst.inventoryitemdata["atlasname"] ~= nil then self.inst.components.inventoryitem.atlasname = self.inst.inventoryitemdata["atlasname"] end
        if self.inst.inventoryitemdata["ondropfn"] ~= nil then self.inst.components.inventoryitem.ondropfn = self.inst.inventoryitemdata["ondropfn"] end
        if self.inst.inventoryitemdata["onpickupfn"] ~= nil then self.inst.components.inventoryitem.onpickupfn = self.inst.inventoryitemdata["onpickupfn"] end
        if self.inst.inventoryitemdata["trappable"] ~= nil then self.inst.components.inventoryitem.trappable = self.inst.inventoryitemdata["trappable"] end
        if self.inst.inventoryitemdata["isnew"] ~= nil then self.inst.components.inventoryitem.isnew = self.inst.inventoryitemdata["isnew"] end
        if self.inst.inventoryitemdata["keepondeath"] ~= nil then self.inst.components.inventoryitem.keepondeath = self.inst.inventoryitemdata["keepondeath"] end
        if self.inst.inventoryitemdata["onactiveitemfn"] ~= nil then self.inst.components.inventoryitem.onactiveitemfn = self.inst.inventoryitemdata["onactiveitemfn"] end
        if self.inst.inventoryitemdata["candrop"] ~= nil then self.inst.components.inventoryitem.candrop = self.inst.inventoryitemdata["candrop"] end
    end  
end
...
function Burnable:IsLightable()
    if (self.inst.components.floatable and self.inst.components.floatable.onwater) or self.disabled then 
        return false 
    end 
