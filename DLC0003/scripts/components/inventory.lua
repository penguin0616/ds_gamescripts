...
function Inventory:IsWearingArmor()
    for k,v in pairs(self.equipslots) do
        if v.components.armor and v.components.armor.condition and v.components.armor.condition > 0 then
            return true
        end
    end
end
...
function Inventory:IsItemNameEquipped(item_name)
   for k,v in pairs(self.equipslots) do
...
function Inventory:SelectActiveItemFromSlot(slot)
    ...
    self.inst:PushEvent("itemlose", {slot = slot, item = newitem})
...
function Inventory:SetNumSlots(num_slots)
    if not num_slots or num_slots <= 0 then
        num_slots = 1
    end
    self.maxslots = num_slots
end
...
function Inventory:RemoveSingleItemBySlot(slot)
...
function Inventory:DropItem(item, wholestack, randomdir, pos, skipfall, tossdir, setspeed)
    if not item or not (item.components.inventoryitem and item.components.inventoryitem.candrop) or item:HasTag("cantdrop") then
        return
    end
    ...
    if dropped then
        ...
        if dropped.components.inventoryitem then
            if self.inst.components.driver and self.inst.components.driver:GetIsDriving() and not tossdir and not dropped.components.inventoryitem.nothrow then                 
...
function Inventory:DropItems(items)
...
function Inventory:GetWaterproofness(slot)
    ...
    local fog = GetSeasonManager().fullfog
    if slot then
        local item = self:GetItemInSlot(slot)
        if item and item.components.waterproofer and (not fog or item:HasTag("fogproof")) then
            waterproofness = waterproofness + item.components.waterproofer:GetEffectiveness()
        end
    else
        for k,v in pairs(self.equipslots) do
            if v and v.components.waterproofer and (not fog or v:HasTag("fogproof")) then
...
function Inventory:TestForOincSound(inst, setnum)
    
    if inst:HasTag("oinc") and not self.inst.SoundEmitter then
        print("RECEIVED OINC BUT DID NOT HAVE SOUND EMITTER",self.inst.prefab)
    end

    if inst:HasTag("oinc") and self.inst.SoundEmitter then
        if not self.inst.oinc_transaction then
            self.inst.oinc_transaction = 0
        end
        local num = 1
        if setnum then
            num = setnum
        elseif inst.components.stackable then 
            num = inst.components.stackable.stacksize
        end
        self.inst.oinc_transaction = self.inst.oinc_transaction + num
        if not self.inst.oinc_transaction_task then
           
            self.inst.oinc_transaction_task =  self.inst:DoTaskInTime(0,function() 
                    if self.inst.oinc_transaction == 1 then
                      self.inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/objects/coins/1")
                    elseif self.inst.oinc_transaction == 2 then
                      self.inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/objects/coins/2")
                    else
                      self.inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/objects/coins/3_plus")
                    end 
                    self.inst.oinc_transaction = nil
                    self.inst.oinc_transaction_task = nil
                end)
        end
    end
end
...
function Inventory:GiveItem( inst, slot, screen_src_pos, skipsound, dontDropOnFail )
    ...
    if not inst.components.inventoryitem or not inst:IsValid() then
        print ("Item inst doesn't have an inventoryitem component or is not valid")
        return
    end


    if not skipsound then
        self:TestForOincSound(inst)
    end
    ...
    if slot then
        ...
        self.inst:PushEvent("itemreceived", {item=inst, slot = slot})
        return slot
    ...
    if not self.activeitem and not TheInput:ControllerAttached() then
        ...
        if self.activeitem and self.activeitem ~= inst and
            ...
            then
            self.activeitem.components.stackable:Put(inst, Vector3(TheSim:GetScreenPos(self.inst.Transform:GetWorldPosition())))
            self.inst:PushEvent("gotnewitem", { item = inst, toactiveitem = true, })

            return true
        else
            if not dontDropOnFail then
...
function Inventory:UnequipAll()
    ...
    for k, item in pairs(self.equipslots) do
       if item and item.components.equippable then
            ...
            self.equipslots[k] = nil
            self.inst:PushEvent("unequip", {item=item, eslot=k})
        end
    end
...
function Inventory:Count(item, checkallcontainers)
    local num_found = 0
    for k,v in pairs(self.itemslots) do
        if v and v.prefab == item then
    ...
    if checkallcontainers then
        ...
        for container_inst in pairs(containers) do
            local container = container_inst.components.container or container_inst.components.inventory
            if container and container ~= overflow and not container.excludefromcrafting then
                num_found = num_found + container:Count(item)
            end
        end
    end
...
function Inventory:Has(item, amount, checkallcontainers)
    local num_found = self:Count(item, checkallcontainers)
    return num_found >= amount, num_found
end

function Inventory:GetSingleItemByName(item)
    for k = 1,self.maxslots do
...
function Inventory:ConsumeByName(item, amount, checkallcontainers)
