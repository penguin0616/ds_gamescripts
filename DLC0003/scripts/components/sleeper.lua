local Sleeper = Class(function(self, inst)
    ...
    self.onlysleepsfromitems = false 
    ...
    self.inst:ListenForEvent("bigfootstep", function(inst, data) self:WakeUp() end, GetWorld())
end)
...
function Sleeper:AddSourceResistance(source, resistance)
    if source and resistance then
        table.insert(self.specific_resistances, {source = resistance})
    end
end

function Sleeper:AddSleepiness(sleepiness, sleeptime, source)
